...
void effect_sobel(Mat input, Mat output);
void effect_canny(Mat input, Mat output, double scale, double th);
...
int main() {
	Mat src, res1, res2;
	src = imread("cap3_02a.bmp");
	imshow("src", src);

	res1 = src.clone();
	res2 = src.clone();

	effect_sobel(src, res1);
	effect_canny(src, res2, 3, 5.0);

	imshow("Sobel", res1);
	imshow("Canny", res2);

	imwrite("sobel.bmp", res1);
	imwrite("canny.bmp", res2);
